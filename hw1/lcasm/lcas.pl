...
print " ** New assembler for LC-2200, version 1.1 **\n";
...
print ".| Read $addr bytes (16 bits each) at origin 0x0000.\n";
...
while ($in = <STEMP>) {
  ...
  if ($in =~ m/^\.word (.*)$/) {
    ...
    if (defined $val) {
      ...
      my $lab = $labels{$1};
      if(!(defined $lab)) {
        # ***** WAS: print STDERR "Label $lab undefined.\n";
        print STDERR "Label $1 undefined.\n";
        exit 7;
      }
      $mem{$addr} = $lab;
    }
...
print ".| Writing labels to $basefile.sym\n";
